...
export function BullionTVDashboard() {
  ...
  return (
    <div className="w-full min-h-screen lg:h-screen lg:max-h-screen bg-[#0a0a0a] text-white overflow-y-auto overflow-x-hidden lg:overflow-hidden flex flex-col p-2 sm:p-4 gap-2 lg:gap-4 font-sans max-w-[100vw] relative">
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-2 lg:gap-4 min-h-0">
        ...
        <div className="w-full lg:w-[45%] flex flex-col gap-2 lg:gap-4 h-auto lg:h-full min-h-[450px] lg:min-h-0">
          <div className="flex-1 border border-[var(--color-gold-500)]/40 rounded-lg flex flex-col bg-[#0f0f0f] overflow-hidden min-h-0">
            <div className="flex justify-between bg-gradient-to-r from-[#d97706] via-[#facc15] to-[#d97706] text-black font-black text-sm sm:text-base lg:text-lg py-2 sm:py-3 px-2 sm:px-4 items-center shadow-[0_4px_15px_rgba(250,204,21,0.4)] border-b-2 border-[#facc15]/60">
              <div className="w-[30%] flex flex-col justify-center">
                <span className="text-left tracking-[0.2em] uppercase drop-shadow-md leading-none">SPOT RATE</span>
                {rates?.timestamp && (
                  <span className="text-[9px] sm:text-[10px] font-mono font-medium tracking-wide text-black/70 mt-1 uppercase">
                    Last updated: {new Date(rates.timestamp).toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute:'2-digit', second:'2-digit' })}
                  </span>
                )}
              </div>
              <span className="w-[35%] text-center tracking-[0.2em] uppercase border-l-2 border-black/60 drop-shadow-md">BID</span>
              <span className="w-[35%] text-center tracking-[0.2em] uppercase border-l-2 border-black/60 drop-shadow-md">ASK</span>
            </div>
            
            <div className="flex-1 p-2 sm:p-4 flex flex-col justify-center gap-y-1 sm:gap-y-2 relative min-h-0">
              {rates?.status === 'stale' && (
                <div className="absolute top-0 right-0 m-2 z-30 bg-red-600/90 text-white text-[10px] sm:text-xs font-bold px-2 py-1 rounded shadow-lg animate-pulse">
                  MARKET DATA STALE
                </div>
              )}
              {isLoading && (
              ...
              <div className="grid grid-cols-[30%_35%_35%] items-center">
                ...
                <div className="flex flex-col items-center px-1 border-l-2 border-black h-full">
                  <FlashBox tick={rates?.timestamp} value={rates?.spotUsd?.gold.ask} className="text-base sm:text-lg lg:text-xl xl:text-3xl font-bold text-white">
              ...
              <div className="grid grid-cols-[30%_35%_35%] items-center">
                ...
                <div className="flex flex-col items-center px-1 border-l-2 border-black h-full">
                  <FlashBox value={rates?.spotUsd?.silver.ask} className="text-base sm:text-lg lg:text-xl xl:text-3xl font-bold text-white">
          ...
          {/* Buyers vs Sellers */}
          <div className="border border-[var(--color-gold-500)]/40 rounded-lg p-4 sm:p-6 h-[100px] lg:h-[130px] bg-[#0f0f0f] flex flex-col justify-center shadow-[0_0_15px_rgba(212,160,28,0.05)] flex-shrink-0">
          ...
          <div className="flex-1 flex flex-col py-1 sm:py-2 lg:py-4 overflow-hidden min-h-0">
            {[
              ...
              <div key={idx} className={`flex items-center px-2 sm:px-4 lg:px-8 flex-1 ${idx !== 4 ? 'border-b border-[var(--color-gold-500)]/10' : ''} hover:bg-white/5 transition-colors`}>
                ...
                <div className="w-[20%] flex items-center justify-center text-xs sm:text-lg lg:text-xl xl:text-2xl text-gray-400 font-black border-l-2 border-black h-full px-1 sm:px-2">{item.weight}</div>
                <div className="w-[25%] flex items-center justify-end text-[13px] sm:text-xl lg:text-2xl xl:text-3xl font-bold tracking-wider font-mono text-white border-l-2 border-black h-full px-1 sm:px-2 whitespace-nowrap">{rates ? fmtAed(item.buy, item.weight === '1 GM' ? 2 : 0) : '...'}</div>
                <div className="w-[25%] flex items-center justify-end text-[13px] sm:text-xl lg:text-2xl xl:text-3xl font-bold tracking-wider font-mono text-white border-l-2 border-black h-full px-1 sm:px-2 whitespace-nowrap">{rates ? fmtAed(item.sell, item.weight === '1 GM' ? 2 : 0) : '...'}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="h-[50px] sm:h-[60px] lg:h-[70px] border border-[var(--color-gold-500)]/40 rounded-lg flex items-stretch overflow-hidden bg-[#0f0f0f] shadow-[0_0_15px_rgba(212,160,28,0.05)] flex-shrink-0 w-full">
