  ...
  return (
    <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 md:p-8 h-full flex flex-col">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-2 mb-6">
        {alerts.map(alert => (
          <div key={alert.id} className="flex items-center justify-between bg-black/5 border border-white/10 p-4 rounded-xl">
            <div>
              <div className="text-sm font-semibold text-white mb-1">{alert.type}</div>
            ...
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-black transition-transform ${alert.active ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </div>
        ))}
      </div>

      <button className="w-full mt-auto bg-black/5 hover:bg-black/10 border border-white/10 border-dashed text-gray-300 p-4 rounded-xl transition-colors flex items-center justify-center gap-2 text-sm font-medium">
        <Plus size={18} />
        CREATE NEW ALERT
      </button>
    </div>
